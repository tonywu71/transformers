...
- **Only the released architecture is expressible** — `use_xsa`, `depth_scale` and `patch_stem` were research ablation switches whose research defaults no released NeoMME uses, so the port implements one value of each and does not expose them at all. The refusal lives in the research repo's conversion script, the only place a research config is read, rather than in a transformers config policing kwargs no other config knows about; the config also carries just the three token ids the forward reads, and one encoding for the window widths. 31 fields down to 24
...
- **Both image-processor backends** — `NeoMMEImageProcessor` (torchvision) and `NeoMMEImageProcessorPil`, emitting flat `pixel_values` plus `image_grid_hw`. Identical with no resolution budget, within one 8-bit level with one, and **`NeoMMEProcessor`** with the marker/query-expansion convention and `score_retrieval`
