...
- **`NeoMMEImageProcessor`** — the torchvision backend only, per maintainer guidance that new models ship the default backend and PIL exists for backwards compatibility. Emits flat `pixel_values` plus `image_grid_hw`, and **`NeoMMEProcessor`** carries the marker/query-expansion convention and `score_retrieval`
