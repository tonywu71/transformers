...
class NeoMMEProcessor(ProcessorMixin):
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        """Apply the text-only retrieval template directly with the tokenizer."""
        if chat_template is None:
            chat_template = self.chat_template
        if not isinstance(chat_template, str):
            raise ValueError("NeoMMEProcessor requires one chat template.")

        call_kwargs = {**(processor_kwargs or {}), **kwargs}
        call_kwargs["tokenize"] = tokenize
