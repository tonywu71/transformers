...
class NeoMMEPatchEmbeddings(nn.Module):
    """Patch stem that maps flattened RGB patches to hidden size."""
...
@auto_docstring(
    custom_intro="""
    NeoMME backbone with a tied masked-language-modeling head.
    """
)
...
class NeoMMEForRetrieval(NeoMMEPreTrainedModel):
    ...
    def get_multivector_embeddings(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        """Return per-token late-interaction embeddings."""
        return self.forward(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
