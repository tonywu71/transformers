...
def get_resize_scale(
    ...
    """Compute the scale to apply to an image before patchifying it.
    ...
    Args:
        ...
        max_side (`int`, *optional*):
            Shrink so the longest side is at most this. Downscale-only.
        max_pixels (`int`, *optional*):
            Shrink so the area is at most this. Downscale-only.
        min_pixels (`int`, *optional*):
            Grow so the area is at least this, the only setting that upscales. Applied first, so a cap
            always wins when the two conflict — which makes the floor best-effort rather than a guarantee:
            the trigger reads the SOURCE area, so `16x16` with `max_side=32, min_pixels=4096` lands at
            `1024` px², a quarter of the floor.

    Returns:
        `float`: The scale to apply. `1.0` leaves the image untouched, which is the default when no cap
        is set, so the patch grid tracks the native resolution.
    """
    scale = 1.0
...
@auto_docstring
class NeoMMEImageProcessor(PilBackend):
    r"""
    Constructs a NeoMME image processor.

    NeoMME is vision-tower-free: an image becomes a row-major grid of FLATTENED `3 * patch_size ** 2` RGB
    patches fed straight to the model's patch stem, so the number of image tokens is decided entirely
    here. By default no resizing happens at all and the grid tracks the image's native resolution; pass
    `max_side`, `max_pixels` or `min_pixels` to bound the patch budget.

    This is a PIL/NumPy processor with no torchvision counterpart, both because the project forbids a
    torchvision dependency and because PIL resizing is what the reference implementation uses — bit-exact
    parity with it is only reachable through this backend.
    """
